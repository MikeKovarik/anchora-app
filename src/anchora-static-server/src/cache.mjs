...
class FsCache extends Map {
	...
	retrieve(desc) {
		return this.get(desc.url)
	}

	store(desc, stream, dependencies) {
		//console.log('--- CACHE', desc.url)
		this.set(desc.url, {desc, stream, dependencies})
	}

	extend(desc, stream, dependencies) {
		//console.log('--- CACHE', desc.url)
		var cacheRecord = this.get(desc.url) || {}
		cacheRecord.desc = desc
		cacheRecord.stream = stream || cacheRecord.stream
		cacheRecord.dependencies = dependencies || cacheRecord.dependencies
		this.set(desc.url, cacheRecord)
	}
